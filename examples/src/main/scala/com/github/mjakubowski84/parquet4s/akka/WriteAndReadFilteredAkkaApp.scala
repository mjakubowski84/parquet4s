...
object WriteAndReadFilteredAkkaApp extends App {
  ...
  for {
    ...
    _ <- ParquetStreams.fromParquet[Data](path, ParquetReader.Options(), Col("dict") === Dict.A).runWith(printingSink)
    _ <- Future(println("""id >= 20 && id < 40"""))
    _ <- ParquetStreams.fromParquet[Data](path, ParquetReader.Options(), Col("id") >= 20 && Col("id") < 40).runWith(printingSink)
    // finish
    _ <- system.terminate()
  } yield ()
