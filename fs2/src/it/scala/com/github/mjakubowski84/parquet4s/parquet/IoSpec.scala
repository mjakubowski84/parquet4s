...
class IoSpec extends AsyncFlatSpec with AsyncIOSpec with  Matchers with PartitionTestUtils with Inside {
  ...
  private def createTempFileAtPath(path: Path): Stream[IO, Path] =
    Stream.eval(Files[IO].createDirectories(path)) >> Stream.resource(
      Files[IO].tempFile(dir = Option(parquetPathToFs2Path(path)), suffix = ".parquet", prefix = "", permissions = None)
    ).as(path)
  ...
  it should "return proper PartitionedDirectory for unpartitioned path with parquet content" in {
    val testStream = for {
      ...
      _ <- Stream.resource(Files[IO].tempFile(dir = Option(parquetPathToFs2Path(basePath)), suffix = ".parquet", prefix = "", permissions = None))
