 ...
 *
 * @tparam In Type of column custom filter refers to
 */
trait UDP[In] {
  /**
   * Used to filter record by record.
   *
   * @param value column value of record that is filtered
   * @return `true` if record containing given value should be kept
   */
   ...
   *
   * @param statistics of the row group
   * @return `true` if the whole row group can be omitted
   */
  def canDrop(statistics: FilterStatistics[In]): Boolean

  /**
   * It is an opposite of `canDrop`. There is a separate function for inverse comparison as the some types may
   * require quite a different logic for that.
   * This function will be called when processing `not` predicates.
   *
   * @param statistics of the row group
   * @return `true` if the whole row group can be omitted for inverse filter
   */
 ...
 * @param min minimum value of `T` in a row group
 * @param max maximum value of `T` in a row group
 * @param ordering [[scala.Ordering]] of `T`
 * @tparam T user type of column
 */
class FilterStatistics[T](val min: T, val max: T)(implicit val ordering: Ordering[T])

private[parquet4s] class UDPAdapter[In, V <: Comparable[V]](udp: UDP[In],
                                                            decoder: FilterDecoder[In, V],
                                                            valueCodecConfiguration: ValueCodecConfiguration
                                                           )(implicit ordering: Ordering[In])
    extends UserDefinedPredicate[V] with Serializable {
