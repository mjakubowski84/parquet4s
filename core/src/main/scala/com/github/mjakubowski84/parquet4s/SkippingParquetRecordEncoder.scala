...
object SkippingParquetRecordEncoder {
  ...
  trait SkippingValueEncoder[T] {
    ...
    def encode(cursor: Cursor, data: T, valueCodec: ValueCodec[T], configuration: ValueCodecConfiguration): Option[Value]

  }

  def defaultSkippingValueEncoder[T]: SkippingValueEncoder[T] = new SkippingValueEncoder[T] {
    override def encode(cursor: Cursor, data: T, valueCodec: ValueCodec[T], configuration: ValueCodecConfiguration): Option[Value] =
