...
object ParquetWriter {
  ...
  private class Builder(path: Path, schema: MessageType)
      extends HadoopParquetWriter.Builder[RowParquetRecord, Builder](path) {
  ...
  def writeAndClose[T](path: String, data: Iterable[T], options: ParquetWriter.Options = ParquetWriter.Options())(
      implicit writerFactory: ParquetWriterFactory[T]
  ): Unit = {
  ...
  def writer[T](path: String, options: ParquetWriter.Options = ParquetWriter.Options())(implicit
  ...
  implicit def writerFactory[T: ParquetRecordEncoder: ParquetSchemaResolver]: ParquetWriterFactory[T] =
    (path, options) => new DefaultParquetWriter[T](path, options)
