...
object RowParquetRecord {
  ...
  def apply(fields: (String, Value)*): RowParquetRecord =
    fields.foldLeft(RowParquetRecord.empty) { case (record, (key, value)) =>
      record.add(key, value)
    }

  /** @return
    *   A new empty instance of [[RowParquetRecord]]
    */
  ...
  implicit def genericSkippingParquetSchemaResolver(implicit
      message: MessageType
  ): SkippingParquetSchemaResolver[RowParquetRecord] =
...
/** Represents a basic type of [[ParquetRecord]] an object that contains a non-empty list of fields with other values
  * associated with each of them. Cannot be empty while being saved. Mutable and <b>NOT</b> thread-safe.
  */
class RowParquetRecord private extends ParquetRecord[(String, Value)] with mutable.Seq[(String, Value)] {
  ...
  /** Encodes the value end appends it to the record.
    */
  def add[T](name: String, value: T, valueCodecConfiguration: ValueCodecConfiguration)(implicit
  ...
  /** Retrieves value from the record and decodes it.
    ...
    *   field/column name
    * @return
    *   decoded field value or `null` if such field does not exist
    */
...
object ListParquetRecord {
  ...
  def apply(elements: Value*): ListParquetRecord =
    elements.foldLeft(ListParquetRecord.empty) { case (record, element) =>
      record.add(ListFieldName, RowParquetRecord(ElementFieldName -> element))
    }
...
/** A type of [[ParquetRecord]] that represents a record holding a repeated amount of entries of the same type. Can be
  * empty. Mutable and <b>NOT</b> thread-safe.
  */
class ListParquetRecord private extends ParquetRecord[Value] with mutable.Seq[Value] {
  ...
  /** @return
    *   collection of elements held in record
    */
  @deprecated("1.1", "Use iterator to iterate or other functions to access or modify record elements.")
...
object MapParquetRecord {
  ...
  def apply(entries: (Value, Value)*): MapParquetRecord =
    entries.foldLeft(MapParquetRecord.empty) { case (record, (key, value)) =>
      record.add(MapKeyValueFieldName, RowParquetRecord(KeyFieldName -> key, ValueFieldName -> value))
    }
...
/** A type of [[ParquetRecord]] that represents a map from one entry type to another. Can be empty. A key entry cannot
  * be null, a value entry can. Mutable and <b>NOT</b> thread-safe.
  */
class MapParquetRecord private
    extends ParquetRecord[(Value, Value)]
  ...
  /** The same as [[update[K,V](K,V,ValueCodecConfiguration)(ValueCodec[K],ValueCodec(V)):This* update]] but returns
    * updated record.
    */
  def add[K, V](key: K, newVal: V, valueCodecConfiguration: ValueCodecConfiguration)(implicit
      kCodec: ValueCodec[K],
      vCodec: ValueCodec[V]
  ): This =
  ...
  /** Updates map with a value for a given key
    ...
    *   the new value
    * @param valueCodecConfiguration
    *   configuration used by some codecs
    * @param kCodec
    *   key codec
    * @param vCodec
    *   value codec
    ...
    *   type of the value
    */
  def update[K, V](key: K, newVal: V, valueCodecConfiguration: ValueCodecConfiguration)(implicit
      kCodec: ValueCodec[K],
      vCodec: ValueCodec[V]
  ): Unit =
