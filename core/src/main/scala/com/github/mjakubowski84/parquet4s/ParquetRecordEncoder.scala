...
object ParquetRecordEncoder {
  ...
  def encode[T](entity: T, configuration: ValueCodecConfiguration = ValueCodecConfiguration.default)(implicit
      ev: ParquetRecordEncoder[T]
  ): RowParquetRecord = ev.encode(entity, configuration)
  ...
  implicit def headValueEncoder[FieldName <: Symbol, Head, Tail <: HList](implicit
    ...
    new ParquetRecordEncoder[FieldType[FieldName, Head] :: Tail] {
      override def encode(
        ...
        val fieldValue =
          try headEncoder.encode(entity.head, configuration)
          catch {
            case NonFatal(cause) =>
