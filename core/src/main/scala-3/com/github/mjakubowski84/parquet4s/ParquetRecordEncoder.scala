...
object ParquetRecordEncoder:
  ...
  def encode[T](entity: T, configuration: ValueCodecConfiguration = ValueCodecConfiguration.Default)
               (using ev: ParquetRecordEncoder[T]): RowParquetRecord = ev.encode(entity, null, configuration)

  given ParquetRecordEncoder[Fields[EmptyTuple, EmptyTuple]] with
    def encode(empty: Fields[EmptyTuple, EmptyTuple],
               resolver: EmptyRowParquetRecordResolver,
               configuration: ValueCodecConfiguration): RowParquetRecord =
  ...
  given [L <: String & Singleton: ValueOf,
    ...
    def encode(fields: Fields[L *: LT, V *: VT],
               resolver: EmptyRowParquetRecordResolver,
               configuration: ValueCodecConfiguration): RowParquetRecord =
  ...
  given derived[P <: Product](using
    ...
    def encode(product: P, resolver: EmptyRowParquetRecordResolver, configuration: ValueCodecConfiguration): RowParquetRecord =
      encoder.encode(
